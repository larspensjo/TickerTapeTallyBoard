...
git commit -m "feat: add SQLite ledger schema and pooled connection"
```
...
git commit -m "feat: add pure ledger domain with weighted-average derivation"
```
...
git commit -m "feat: add instrument and transaction CRUD API"
```
...
git commit -m "feat: add derived holdings endpoint"
```
...
git commit -m "feat: wire manual entry UI to the ledger API"
```
...
git commit -m "docs: record Phase 1 ledger decisions"
```
